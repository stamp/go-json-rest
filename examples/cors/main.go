...
func main() {
	...
	handler := rest.ResourceHandler{
		EnableCors: func(info *rest.CorsRequestInfo) *rest.CorsResponseHeaders {

			res := &rest.CorsResponseHeaders{}

			if !info.IsCors {
				// be nice with non CORS
				return res
			}

			if info.Origin != "http://my.other.host" {
				// allow only this host
				return nil
			}

			if info.IsPreflight {
				// TODO check the request methods
				// TODO check the request headers
				// TODO populate res
				return res
			} else {
				// TODO populate res
